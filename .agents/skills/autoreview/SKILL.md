...
| **droid** | `droid exec --cwd <repo>` from a neutral subprocess cwd, default read-only mode verified from help, plus disabled shell/write/workflow tools | Droid CLI `exec --help` |
...
- runs Droid from a neutral temporary directory after verifying its read-only/tool-control help text, disables shell/write/workflow tools, forwards `--model` and `-r, --reasoning-effort`, and switches `--output-format` to `stream-json` when streaming is enabled
